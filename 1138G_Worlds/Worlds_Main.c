...
/*
Changelog:
AD - Added table for skills challenge and edited skills challenge code (Magic number is 45)
AD - includes Worlds_Functions and LCD_Display ----- Worlds_Main_TEST is for test purposes only!
*/
...
Joystick Map:
						*****Base controller*****
								Left joystick controls left side of bot
								Right joystick controls right side of bot.
								Right 8Up button controls winch mechanism - makes it go up
								Right 8D button controls winch mechanism - makes it go down

						*****Partner Controller*****
								Left joystick controls intake and conveyors
								Right trigger Up controls flywheel speed - High Speed
								Right trigger Down controls flywheel speed - Half court Speed
								Right 8D button controls flywheel speed - Bar speed
*/
	...
	while(true)
	{
		...
		if(timerLock > 250)
		{
			...
			if(vexRT(Btn6UXmtr2) == 1 && vexRT(Btn6DXmtr2) == 0 && vexRT(Btn8DXmtr2) == 0) 			//Full court -- 6U = faster
				...
				if(vexRT(Btn7UXmtr2) == 1 && vexRT(Btn7UXmtr2) == 0)
				{
					Skills_Autonomous();
				}
